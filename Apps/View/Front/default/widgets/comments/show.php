    ...
    <div class="col-md-12">
        <div class="page-comment">
            <ul class="comments" id="comment-list">
                <li class="d-none" id="load-comments"></li>
            </ul>
        </div>
...
<li class="clearfix d-none" id="comment-structure">
        ...
        <div class="col-md-12" style="padding-bottom: 15px;">
            <div class="pull-right">
                <a href="#showReplay" class="btn btn-info btn-sm comment-hook-replay" id="replay-to-0">
                    <i class="fa fa-quote-left"></i> <?= __('Replay') ?>
                </a>
            </div>
        </div>
    </div>
...
<ul class="comments d-none" id="comment-answer-item">
    <li class="clearfix">
...
<?php if ((bool)$configs['guestAdd'] || \App::$User->isAuth()): ?>
    ...
    <div class="row" id="showReplay">
        <div class="col-md-12">
        <span class="badge badge-primary d-none" id="replay-to-text"><?= __('Replay to') ?>:
...
<script>
    $(document).ready(function() {
        $(function () {
            ...
            comStructure.removeClass('d-none').removeAttr('id');
            answStructure.removeClass('d-none').removeAttr('id');
            comSendForm.removeClass('d-none').removeAttr('id');
            ...
            buildCommentDOM = function (data) {
                ...
                if (data.moderate === 1) {
                    ...
                    commentDom.find('#comment-date').before('<span class="badge badge-danger"><?= __('On moderation') ?></span> ');
                }
                // set comment text
            ...
            buildAnswerDOM = function (data) {
                ...
                if (data.moderate === 1) {
                    ...
                    ansDom.find('#answer-date').before('<span class="badge badge-danger"><?= __('On moderation') ?></span> ');
                }
            ...
            loadCommentList = function() {
                $.getJSON(script_url+'/api/comments/list/' + comOffset +'?path=' + comPath + '&lang='+script_lang, function (json) {
                    ...
                    if (json.leftCount > 0) {
                        $('#comment-show-more').removeClass('d-none').find('#comment-left-count').text(json.leftCount);
                    } else {
                        $('#comment-show-more').addClass('d-none');
                    }
                });
                // increase offset
                comOffset++;
            ...
            loadAnswerList = function (comId) {
                ...
                $.getJSON(script_url + '/api/comments/showanswers/' + comId + '?lang='+script_lang, function(json){
                    if (json.status !== 1) {
                        targetElement.parent().find('#add-replay-to').hide().removeClass('d-none').show('slow');
                        return null;
                    }
                    ...
                    $.each(json.data, function(idx,row){
                        ...
                        // make item visible
                        targetElement.hide().removeClass('d-none').show('slow');
                        // save marker
            ...
            $(document).on('click', '.comment-hook-replay', function(){
                ...
                if (comId > 0) {
                    ...
                    comHtml.hide().removeClass('d-none').show('slow');
                }
            });
                ...
                $.post(script_url+'/api/comments/add?lang='+script_lang, formData.serialize()).done(function(res){
                    ...
                    if (res.status === 1) {
                        ...
                        if (res.data.type === 'post') {
                            ...
                            targetElem.find('#load-comments').before(domHtmlComment);
                            ...
                            formData.trigger('reset');
                            editorCleanup();
                            targetCommentDom.append(domHtmlAnswer).hide().removeClass('d-none').show('slow');
                        }
                    } else {
                        alert(res.message);
                    }
            ...
            var editorCleanup = function () {
                // tinymce can cleanup themselv, no reason
                //alert('cleanup');
            };
