...
return [
    ...
    'From' => 'От кого',
    'DSN connection' => 'DSN строка',
    'Set email from which one users will receive messages. Example: zenn1989@gmail.com' => 'Укажите с какой почты будут отправлены сообщения. Пример: zenn1989@gmail.com',
    'Configure DSN connection string to your email provider. See more at Symfony mailer. Example: smtp://user:pass@smtp.example.com:port' => 'Настройте строку по стандарту DSN подключения для вашего email провайдера. Более подбробно - symfony mailer. Например: smtp://user:pass@smtp.example.com:port',
    ...
    'Connect Yandex.Metrika' => 'Подключить Яндекс.Метрику',
    'Views' => 'Просмотры',
    'Bounces, %' => 'Отказы, %',
    'Users, page views, bounces - 30 days' => 'Пользователи, визиты, отказы за 30 дней',
    'Traffic sources - 30 days' => 'Источники трафика за 30 дней',
    'Yandex.metrika - connection' => 'Яндекс.Метрика - подключение',
    'Yandex metrika - step 1' => 'Яндекс метрика - шаг 1',
    'Connect yandex metrika - step 1' => 'Подключение к яндекс метрике - шаг 1',
    'To make successful connection to yandex.api you should follow next steps:' => 'Для осуществления подключения к yandex.api следуйте следующей инструкции:',
    'Create yandex metrika counter' => 'Создайте счетчик в сервисе яндекс.метрика',
    'Create yandex oauth application and put application ID in form down' => 'Создайте специальное oauth приложение и поместите его ID в форму ниже',
    'On yandex website in section general check "Web-service", in section "Yandex.Metrika" check option "Get statistics, read params".' => 'На сайте яндекса в разделе главное отметьте галочкой "Веб-сервис", в секции "Яндекс.Метрика" отметьте опцию "Получать статистику, читать параметры"',
    'Create new yandex oauth app' => 'Создать новое приложение yandex oauth',
    'Set callback uri #1 to (use https or fail):' => 'Укажите callback uri #1 на сайте яндекса равный (только https!):',
    'Input created application ID (name) there (not a secret password)' => 'Введине ID созданного приложения (имя) в это поле (не пароль!)',
    'After create application - initialize token click button below' => 'После создания приложения и указания его ID - инициируйте процесс получения токена кнопкой ниже',
    'Get Yandex.Metrika token' => 'Получить токен Yandex.Metrika',
    'Update' => 'Обновить',
    'Create oauth application before get token' => 'Создайте oauth приложение перед тем как получать токен',
    'Application ID saved' => 'ID приложения успешно сохранен',
    'Yandex.Metrika - token' => 'Яндекс.метрика - токен',
    'Yandex metrika - step 2' => 'Яндекс метрика - шаг 2',
    'This is callback form - you are successfully got a new yandex token! Save it!' => 'Это callback форма получения токена - вы получили ключ! Сохраните его!',
    'Token saved successful. Valid until: %date%' => 'Токен успешно сохранен. Действителен до: %date%',
    'Yandex.Metrika - counter' => 'Яндекс.метрика - счетчик',
    ...
    'permanent' => 'постоянно',
    'Ban update' => 'Управление блокировкой',
    'IP address' => 'IP адрес',
    'User id' => 'ID пользователя',
    'Read block' => 'Заблокировать чтение',
    'Block website read? Attention: user will not be permitted to see website content!' => 'Заблокировать возможность читать сайт? Внимание: пользователь не сможет просматривать никакие материалы сайта!',
    'Write block' => 'Заблокировать запись',
    'Block website write?' => 'Заблокировать возможность писать на сайте?',
    'Set block expires date or left empty to permaban' => 'Укажите дату, когда истечет блокировка или оставьте пустым для блокировки навсегда',
    'Ban expires' => 'Блокировка до',
